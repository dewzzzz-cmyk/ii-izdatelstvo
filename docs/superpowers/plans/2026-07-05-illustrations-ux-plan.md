...
⚠ Номер строки (1557) актуален только ДО применения Task 2 (Task 2 добавляет строки внутрь `exportPdf`, который идёт выше `renderEdit` в файле — всё, что ниже, сдвигается). Искать нужно по цитируемому коду ниже, а не по номеру строки.
