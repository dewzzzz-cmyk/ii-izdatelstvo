...
Найти в `renderWrite` место, где привязываются обработчики после установки `els.center.innerHTML` (рядом с другими `document.getElementById(...)`-биндингами для этой же функции — искать `edUndo`/`edRedo` или `brief` чуть ниже установленного `innerHTML`), добавить:
```js
