...
async function generateConcept(silent){
  ...
  const sys=`Ты — редактор-разработчик. Создай детальный ЗАМЫСЕЛ книги. Верни СТРОГО JSON:\n{${settingInstruction},"characters":[{"name":"Имя Фамилия","age":"возраст","role":"роль в истории","brief":"суть персонажа — 1 предложение","secret":"главная тайна или скрытый мотив — конкретно","detail":"одна живая деталь: жест/привычка/черта внешности","motive":"чего хочет в этой истории — конкретно"}],"plotTurns":"3-5 конкретных поворотов. ПЕРВЫЙ поворот — это обязательно момент когда обычный мир разрушается (письмо, событие, открытие). Не начинай с уже запущенного действия.","tone":"тон и настроение"}. Только JSON. Персонажей 3-5, все с заполненными полями.`;
