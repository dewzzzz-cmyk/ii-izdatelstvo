...
function cleanProse(n){
  ...
  // Снимаем типичные редакторские преамбулы ДО текста:
  // «Вот (отредактированный/вычитанный/исправленный/готовый) текст…\n\n***\n»
  let s=out;
  s=s.replace(/^[^\n]*(?:вот\s+\S*\s*текст|ниже\s+(?:исправлен|готов|отредактирован)|текст\s+готов)[^\n]*\n+/i,'');
  s=s.replace(/^\*{2}[^*\n]+\*{2}:?\s*\n+/,'');  // **Исправленный текст:**
