...
const TEMPLATES = [
  ...
  { role:'audit', name:'Аудитор',       title:'Детектор противоречий', emoji:'🕵️',
    prompt:'Ты — аналитик нарратива. Твоя ЕДИНСТВЕННАЯ задача — составить список проблем. НЕ ИСПРАВЛЯЙ текст.\n\nЧТО ИСКАТЬ:\n1. Атрибуция действий: персонаж A совершил действие, но другой персонаж обвиняет/благодарит персонажа C — кто реально совершил действие?\n2. Унаследованные реплики/ситуации из источника, не адаптированные под изменённый сюжет (сверяй с брифом проекта)\n3. Пространственные скачки: персонаж был в точке A, оказался в точке B без объяснения перемещения\n4. Временны́е противоречия: порядок действий нарушен в пределах сцены\n5. Брошенные персонажи: появился в важной сцене и пропал без следа\n\nФОРМАТ ОТВЕТА — строго:\n===ПРОБЛЕМЫ===\n[N] <где в тексте>: <одна строка описания>\n===КОНЕЦ===\n\nЕсли проблем нет — верни:\n===ПРОБЛЕМЫ===\nПроблем не найдено\n===КОНЕЦ===' },
  { role:'logedit', name:'Логред',      title:'Редактор логики', emoji:'🔎',
    prompt:'Ты — редактор логики. В сообщениях выше ты получил СПИСОК ПРОБЛЕМ от Аудитора (блок ===ПРОБЛЕМЫ===...===КОНЕЦ===). Используй его.\n\nАЛГОРИТМ:\n1. Найди блок ===ПРОБЛЕМЫ===...===КОНЕЦ=== в предыдущих сообщениях. Если там «Проблем не найдено» — переходи к шагу 3 сразу.\n2. Для каждой проблемы из списка: определи истинную версию события (канон = первое упоминание если нет авторской ремарки), исправь ВСЕ места где нарушена эта истина — диалоги, реакции, местоимения.\n3. Также найди и исправь любые другие нестыковки (пространство, время), которые аудитор мог пропустить.\n\nФОРМАТ ОТВЕТА — строго:\n===ПРАВКИ===\n[N] <что именно изменено — одна строка>\n===ТЕКСТ===\n[полный исправленный текст]\n===КОНЕЦ===\n\nЕсли правок нет:\n===ПРАВКИ===\nПравок нет\n===ТЕКСТ===\n[текст без изменений]\n===КОНЕЦ===\n\nОбъём текста — не менее 95% от оригинала.' },
...
function renderWizConcept(){
  ...
  if(!hasContent){
    ...
    const plotPreview=plotTurns.split('\n').filter(Boolean).slice(0,3).map(l=>`<div class="wc-turn">• ${esc(l.replace(/^\d+\.\s*/,'').slice(0,120))}</div>`).join('');
    body.innerHTML=`
...
function renderSimpleProgress(){
  ...
  const head=finished
    ? `<div class="sp-head">📖 Книга готова — ${doneCount} из ${nodes.length} этапов · ${_elStr}</div>`
    : running
