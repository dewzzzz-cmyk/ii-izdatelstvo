...
import { topRepeatedStems } from './bible.js';
...
export function evaluatorMessages(scene, draft, voiceExamples, bookContext, rules){
  const sys = [
    ...
    'Тебе дан ниже АВТОМАТИЧЕСКИЙ ПОДСЧЁТ повторов слов-корней по всей сцене (посчитано кодом, не на глаз) — используй его для «Свежести»: если частый корень — это навязчивый стилистический тик (деталь/звук/жест, который автор возвращает почти теми же словами вместо развития), процитируй конкретный повтор и оштрафуй как клише; если частота объясняется тем, что это имя персонажа или центральный предмет/термин сюжета — не штрафуй.',
  ...
  const repeatsBlock = repeats.length
    ? '\nАвтоматический подсчёт повторов (пример формы слова × кол-во раз этого корня по всей сцене, без стоп-слов): ' + repeats.map(r=>`«${r.example}»×${r.count}`).join(', ') + '\n'
    : '';
  const user = [
    'Бриф сцены: ' + (scene.brief || scene.title || ''),
