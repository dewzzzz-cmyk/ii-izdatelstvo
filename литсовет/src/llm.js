...
// Общий потолок на длину ОТВЕТА для всех повторов «с удвоенным лимитом».
// Раньше в каждом месте стояло своё число (3000, 19200, 24000), и после
// подъёма дефолтов эти числа оказывались НИЖЕ базового лимита агента: тогда
// Math.min(потолок, база*2) отдавал меньше базы, а спасавший Math.max(база+1)
// превращал «повтор вдвое шире» в «повтор на один токен шире» — то есть в
// бессмыслицу, которая молча съедала повторную попытку. Одна константа выше
// любого нашего дефолта убирает этот класс целиком.
// ВНИМАНИЕ: у провайдеров свои пределы (DeepSeek V4 — 384 000 на выход,
// у Anthropic заметно меньше). Здесь берём заведомо безопасное значение.
export const MAX_OUTPUT_TOKENS = 120000;
