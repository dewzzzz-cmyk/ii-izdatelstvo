...
import { genresOf, ageProseNote, ageContentNote, genreWorldNote, multiGenreNote } from './genres.js';
...
// Принимает строку (как раньше) или список жанров книги. У книги «фэнтези +
// ЛитРПГ» нужны ОБЕ спец-категории: до мультижанра сюда приходил только ведущий
// жанр, и вторая категория молча не создавалась — автор не видел, что канон
// системы прогрессии вообще не собирается.
export function categoriesFor(genre){
  const список = Array.isArray(genre) ? genre : [genre];
  const строки = список.filter(Boolean).map(x=>String(x).toLowerCase());
  const cats = [...BASE_CATEGORIES];
  // Порядок вставки обратный, потому что обе идут на одну позицию (1): так
  // «система» окажется перед «магией/технологией», если есть обе.
  if(строки.some(g=>genreWantsWorld(g) && !g.includes('литрпг'))) cats.splice(1, 0, 'магия/технология');
  if(строки.some(g=>g.includes('литрпг'))) cats.splice(1, 0, 'система');
  return cats;
}
...
// Возрастная поправка для канона мира. Мир детской книги строится вокруг того,
// что ребёнок может увидеть и потрогать, а не вокруг систем управления.
function ageWorldNote(age){
  const a = String(age||'');
  if(a==='3-6' || a==='7-10'){
    return `ВОЗРАСТ ЧИТАТЕЛЯ (${a==='3-6'?'3–6':'7–10'} лет): факты мира должны быть КОНКРЕТНЫМИ и наглядными — то, что герой может увидеть, потрогать, съесть, испугаться. Никакой геополитики, экономики, систем власти, войн и массовых жертв: вместо «две фракции борются за торговый путь» — «в лесу живёт барсук, который прячет чужие вещи в дупле». Правила мира формулируй как простые и нерушимые («в этом доме нельзя открывать синюю дверь»), без исключений и оговорок. Жестокость, смерть и увечья из канона исключены.`;
  }
  if(a==='11-14'){
    return 'ВОЗРАСТ ЧИТАТЕЛЯ (11–14 лет): мир может иметь конфликт сил и историю с потерями, но объясняй его через то, что видит подросток-герой (школа, двор, семья, отряд), а не через справку о государственном устройстве. Абстрактные системы допустимы, если у них есть видимое бытовое проявление.';
  }
  return '';
}
...
// Принимает строку или список жанров: при мультижанре «альтернативная история»
// может стоять вторым или третьим, и проверка только по p.genre молча выключала
// требование точки развилки — жанр выбран, а его главное правило не доезжало.
function altHistoryNote(genre){
  const список = Array.isArray(genre) ? genre : [genre];
  const g = список.filter(Boolean).map(x=>String(x).toLowerCase()).join(' ');
...
export function worldSuggestMessages(state, category, opts={}){
  ...
  const sys = [
    ...
    // Промпт требовал не противоречить СУЩЕСТВУЮЩЕМУ канону и другим
    // категориям — и ни слова о согласованности фактов ВНУТРИ одной выдачи.
    // Живой прогон: в одном ответе про героиню пришли «она — сущность, её тело
    // регенерирует каждые 20 лет», «у неё шрам от старой травмы», «ею движет
    // альтер-эго по имени Тень» и «она предотвращает собственное убийство» —
    // четыре разные книги. Кнопка при этом предлагает «Сохранить в канон (8)»,
    // то есть добавить их ВСЕ разом, и автор одним кликом сеет противоречия.
    'ГЛАВНОЕ: все факты в твоём ответе должны быть верны ОДНОВРЕМЕННО, в одном и том же мире. Это связный набор, а не список альтернатив на выбор. Если у тебя есть несколько взаимоисключающих идей — выбери ОДНУ, самую сильную, и разверни её другими фактами, а остальные не предлагай. Проверь свой список перед ответом: если два факта не могут быть истинны вместе, один из них лишний.',
    ...
    altHistoryNote(genresOf(p)),
    // Какого ТИПА факты нужны книге именно этого жанра. До этого жанр решал
    // только набор категорий (categoriesFor), но не наполнение: мир сказки и
    // мир детектива собирались одинаково — справочником государств и дат.
    multiGenreNote(genresOf(p), genreWorldNote, 'МИР И ЖАНР — какие факты вообще нужны этой книге:'),
    // Возраст читателя: мир детской книги — не уменьшенный взрослый. Без этого
    // Архивариус ровно так же сочинял геополитику, экономику и жестокую историю
    // для книги шестилетним, а Прозаик потом обязан был вносить это в сцены.
    ageWorldNote(p.ageGroup),
  ].filter(Boolean).join('\n');
  const user = [
...
export function missingPOD(state){
  // По всем жанрам книги, а не только по ведущему: «фэнтези + альтернативная
  // история» раньше не проверялось вовсе.
  const g = genresOf(state.project||{}).join(' ').toLowerCase();
...
export async function suggestMissingWorldFacts(state, skeleton){
  ...
  try{
    ...
    const cats = categoriesFor(genresOf(state.project||{}));
    return arr.slice(0,12).map((f,i)=>({
