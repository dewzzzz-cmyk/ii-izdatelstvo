...
// «персонажи» — универсальная категория (в отличие от «магия/технология»/
// «система» ниже, актуальна для ЛЮБОГО жанра: скрытая природа персонажа
// (не человек/иной вид/секретная способность), которую сцены держат в уме,
// даже если сам персонаж её не раскрывает окружающим — то, что раньше не
// извлекалось ни Книжным архитектором (он только ЧИТАЕТ канон, не пишет
// его), ни Стражами (они видят только structured-факты, не сырой синопсис),
// и жило только в синопсисе автора, недоступное проверке до первой сцены.
const BASE_CATEGORIES = ['персонажи', 'география', 'история', 'фракции', 'культура'];
...
export const CATEGORY_HINTS = {
  'персонажи': 'скрытая природа персонажей, если она НЕ то, чем кажется на первый взгляд (не человек, иной вид, секретная способность, двойная личность) — по одному конкретному, проверяемому факту на персонажа, где это уместно; не факты о внешности/характере, которые не влияют на то, что физически возможно или невозможно в сцене с этим персонажем.',
...
export function mapPromptFor(state){
  ...
  const geoLine = autoLabels
    ...
    : `Geography (must appear as visual features — ${facts}). Label ONLY the ${labelCount} most important named place${labelCount>1?'s':''}. Each label is TWO lines: on top, a SMALL, noticeably less prominent line — exactly ONE plain Russian word naming its general terrain type (горы, пустыня, лес, болото, озеро, остров, город, река, побережье, долина, etc. — whichever matches the fact); directly under it, in LARGE, bold, hand-lettered text (${MAP_LANGUAGES[lang].instr}) styled as part of the map's decoration (not a printed caption), the specific name. The name must be SHORT — one word, or a two-word nickname, never the full name. ALWAYS strip the generic category word from the name, keeping only the distinctive part — if a place's full name starts with a generic terrain-type word (a desert, mountain range, swamp, forest, lake, etc.) followed by a distinctive part, drop that leading generic word from the label and keep only the distinctive remainder — this applies to EVERY label, not just some of them. This is not optional: the type line above already states that generic word (горы, пустыня, болото, лес...), so keeping it in the name too means saying the same thing twice on the same label. Never merge two different place names into one invented hybrid label — each label names exactly ONE place from the facts above. Do not add a title, banner, caption, scale bar, or any other text on the map beyond these ${labelCount} name+type pairs — the ONLY exception is a compass rose's single-letter N/S/E/W marks, nothing else. ${fontNote}${riskNote}`;
  return [
