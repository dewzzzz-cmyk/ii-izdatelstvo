...
export function addCustomAgent(state, name, prompt){
  const a = { id:'custom_'+(Date.now().toString(36))+(_agc++), name:name||'Свой страж', icon:'🛡',
    temp:0.2, maxTokens:2520, strictness:2, enabled:true, role:'custom', custom:true,
...
function migrate(s){
  ...
  // Второй раунд той же настройки, в другую сторону: теперь литературные
  // стражи бегут с первой итерации (pipeline.js), и 5 попыток избыточны —
  // если автор ещё сидит именно на 5 (старый дефолт этого раунда, не ручная
  // правка), спускаем на новый дефолт 3.
  if(s.global.evaluatorMaxIter === 5) s.global.evaluatorMaxIter = 3;
  // Однократный спуск 3→1 для structureMaxIter — та же логика, что и выше:
  // трогаем, только если автор ещё сидит на старом дефолте (3), ручную
  // настройку не перезаписываем. См. комментарий у поля в defaultState().
