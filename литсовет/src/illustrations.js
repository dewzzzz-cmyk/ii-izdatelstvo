...
// Наложен ли уже НАСТОЯЩИЙ текст автора на текущую обложку (compositeCoverTitle) —
// используется экспортом (.pdf/.doc), чтобы не печатать имя автора ВТОРОЙ раз
// отдельной подписью под обложкой. baseDataUrl непустой = обложка прошла путь
// композитинга (см. generateIllustrationFor); title/author непустые = именно
// они и были нарисованы (compositeCoverTitle ничего не рисует без названия,
// а без автора рисует только название). Не учитывает более редкий случай, когда
// автора вписали ПОСЛЕ последней генерации/«Обновить название» — тогда картинка
// ещё не в курсе, и на экспорте автор пропадёт из подписи, но не появится и
// на самой обложке, пока не нажать «🔤 Название» ещё раз — тот же принцип
// «наложение по клику, не автоматически», что и у всей этой фичи.
export function coverHasBakedAuthor(state){
  const item = (state.illustrations?.items||[]).find(it=>it.type==='cover');
  return !!(state.project?.coverDataUrl && item?.baseDataUrl && state.project?.title && state.project?.author);
}
