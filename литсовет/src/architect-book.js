...
// Правило, которого не было вовсе, — а нарушалось оно в каждом длинном скелете.
// «Грезы»: Морок побеждён в гл. 7, потом снова в гл. 11, потом снова в гл. 14;
// герой возвращается в Пустошь трижды. Во втором прогоне буквальные повторы
// ушли, но охота за артефактом всё равно шла три раза под разными вывесками
// («Хранилище и Кристалл Миров» → «Потерянное заклинание» → «Горы и артефакт»).
// Ни ARC_DISTRIBUTION_NOTE, ни BRIEF_CHOICE_NOTE этого не запрещали: они
// говорят про дуги и про отдельную сцену, но ничего — про отношение главы к
// уже написанным главам. Кодом это не ловится: проверка на одинаковые названия
// обходится переименованием, а порога по смысловой близости не существует —
// «кульминация ↔ развязка» похожи ровно так же сильно (0.77), как настоящий
// повтор, и любая отсечка забракует законную пару.
const NO_LOOP_NOTE = 'НЕОБРАТИМОСТЬ: каждая глава обязана менять положение героя так, чтобы вернуться к прежнему состоянию было уже нельзя. Книга не должна дважды проходить одну и ту же точку.'
  + ' Запрещено: побеждать одного и того же противника больше одного раза; повторно добывать, терять и снова добывать один и тот же предмет; возвращать героя в место, из которого он уже уходил, если там его ждёт та же задача; заново узнавать то, что он уже знает.'
  + ' Переименование не считается решением: «поиск артефакта», «потерянное заклинание» и «путь к хранилищу» — это одна и та же глава под тремя вывесками. Проверь итоговый список глав как список: если два пункта можно пересказать одной фразой, это повтор.'
  + ' Если сюжета не хватает на запрошенное число глав — не разбавляй его витками одного и того же. СОКРАТИ число глав и укрупни события: десять глав, где каждая меняет положение необратимо, лучше восемнадцати с тремя одинаковыми витками. Ты имеешь право вернуть меньше глав, чем запрошено, если так книга становится плотнее.'
  + ' Противник, если он один на всю книгу, должен между появлениями МЕНЯТЬСЯ: узнавать о герое больше, менять цель, поднимать ставку, терять или приобретать союзников. Одинаковое столкновение дважды — это не нагнетание, а топтание.';
...
export function bookArchitectMessages(state, opts={}){
  ...
  const user = [
    ...
    // Здесь стояло «~N сцен на главу», и модель понимала это буквально: в живом
    // скелете «Грез» ВСЕ 17 глав получили ровно по 4 сцены, во втором прогоне —
    // тоже ровно по 4. Метроном создавала не модель, а эта строка: числом на
    // главу мы сами задавали равномерность, которую потом Оценщик штрафовал по
    // оси «баланс». Даём бюджет на книгу и требуем разброса.
    `Раздели на ${targetChapters} глав. Всего сцен в книге — около ${targetChapters*scenesPerCh}; распределяй их по главам НЕРАВНОМЕРНО, по драматической нужде: глава-поворот или кульминация требует больше сцен, переход или передышка — меньше. Разброс обязателен: если у всех глав вышло одинаковое число сцен, ты расставлял их по счётчику, а не по событиям — переделай. Ориентир: от ${Math.max(2, scenesPerCh-2)} до ${scenesPerCh+2} сцен в главе.`,
...
export function bookArchitectPatchMessages(state, { affectedChapters, hint }){
  ...
  const sys = [
    ...
    // Точечная правка тоже умеет завести виток: модель видит только несколько глав и
    // легко повторяет то, что уже было в невидимой ей части книги.
    NO_LOOP_NOTE,
