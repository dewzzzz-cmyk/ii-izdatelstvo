...
export function bookArchitectMessages(state, opts={}){
  ...
  const user = [
    ...
    '{ "chapters": [ { "title": "название главы", "arc": "завязка|развитие|кульминация|развязка", "scenes": [ { "title": "название сцены", "brief": "3-4 предложения: где происходит + кто участвует + ключевое событие/конфликт + чем заканчивается сцена и что меняется", "emotion": "эмоция читателя в финале сцены", "targetWords": число } ] } ] }',
    `Спланируй ~${targetChapters} глав, в каждой ~${scenesPerCh} сцен. Итого ~${targetScenes} сцен. Сумма targetWords всех сцен должна быть около ${totalWords}. Брифы конкретные и детальные — место, персонажи, поворот. Только JSON.`,
  ].filter(Boolean).join('\n');
