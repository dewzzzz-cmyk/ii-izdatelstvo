...
export function validateOutline(raw){
  ...
  // Проверка дуг идёт ПОСЛЕ проверки на главы-наполнители: если половина глав
  // пустые, причина съехавших дуг именно в этом, и говорить надо о ней.
  const дуги = проверьДуги(chapters);
  if(дуги) return { ok:false, kind:'content', error:дуги };
...
// ARC_DISTRIBUTION_NOTE существует с самого начала, но проверялась только тем,
// послушалась ли модель, — в коде правило дуг не проверял никто. Живой прогон
// (детектив, 16 глав) вскрыл это так: пролог → завязка → развитие ×13 →
// КУЛЬМИНАЦИЯ → ЭПИЛОГ. Развязки нет вообще. Промпт велит ставить кульминацию
// предпоследней при 4+ главах, и модель честно поставила её предпоследней —
// но предпоследней среди ВСЕХ глав, считая эпилог. Развязка оказалась вытеснена.
// То есть добавление рамочных глав (1.83.0) сделало этот провал достижимым, а
// поймать его было нечем: книга приходила без финала, и узнать об этом можно
// было только прочитав скелет глазами.
function проверьДуги(chapters){
  const тело = chapters.filter(c=>!FRAME_ARCS.has(c.arc));
  if(тело.length < 3) return '';   // на 1-2 главах говорить о полной арке нечего
  const счёт = a => тело.filter(c=>c.arc===a).length;
  if(счёт('развязка') === 0){
    return 'в книге нет главы с дугой «развязка» — книга обрывается на кульминации. Эпилог развязку не заменяет: он идёт ПОСЛЕ неё и не разрешает конфликт';
  }
  if(счёт('развязка') > 1) return `глав с дугой «развязка» — ${счёт('развязка')}, должна быть ровно одна (последняя в основной части)`;
  if(счёт('кульминация') > 1) return `глав с дугой «кульминация» — ${счёт('кульминация')}, должна быть ровно одна`;
  // При ровно трёх главах кульминация совпадает с развязкой — так прямо
  // сказано в ARC_DISTRIBUTION_NOTE, и требовать отдельную главу-пик здесь
  // значит противоречить собственной инструкции.
  if(тело.length >= 4 && счёт('кульминация') === 0){
    return 'в книге нет главы с дугой «кульминация» — нет пика';
  }
  if(тело[тело.length-1].arc !== 'развязка'){
    return `последняя глава основной части («${тело[тело.length-1].title}») имеет дугу «${тело[тело.length-1].arc}», а должна быть «развязка»`;
  }
  if(тело[0].arc !== 'завязка'){
    return `первая глава основной части («${тело[0].title}») имеет дугу «${тело[0].arc}», а должна быть «завязка»`;
  }
  return '';
}
