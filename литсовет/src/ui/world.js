...
export function renderWorld(els){
  const s = getState();
  // Гвард стадии. Внутри этого файла renderWorld(els) вызывается из ~40 мест, и
  // почти все — из ЗАВЕРШЕНИЙ асинхронных операций («Предложить весь мир» — это
  // 5-6 платных запросов подряд, генерация карты, распознавание меток, оценка
  // глубины, поиск нестыковок). Вызывались они напрямую, а не через диспетчер
  // renderStage() (ui/app.js) — единственное место, которое сверяется с
  // s.ui.stage. Из-за этого автор, нажавший «Дальше — Структура →» не дожидаясь
  // ответов, получал следующее: каждый прилетающий ответ затирал els.left/
  // center/right разметкой вкладки «Мир» — экран несколько раз подряд
  // выдёргивало обратно, а всё, что автор успел напечатать на «Структуре»,
  // терялось. Ровно тот же класс бага, что уже чинили с подписями карты.
  // Проверка стоит здесь, а не на 40 вызывающих местах: так её невозможно
  // забыть в новом коде. Мутации state при этом уже применены — теряется только
  // ненужная перерисовка, и при возврате на «Мир» диспетчер отрисует всё свежим.
