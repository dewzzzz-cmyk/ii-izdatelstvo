...
function bindChat(){
  ...
  if(applyBtn) applyBtn.onclick = ()=>{
    ...
    (async ()=>{
      try{
        // Возврат проверяем: summarizeScene отдаёт null и без исключения —
        // например, при нераспознаваемом ответе архивариуса. Молчание здесь
        // значило бы, что переписанная сцена не попала в память, а автор
        // узнал бы об этом только по расхождениям в следующих сценах.
        const sum = await summarizeScene(s, scene);
        scene.drift = driftCheck(s, scene);
        await maybeRollup(s);
        save();
        if(!sum) alert('Текст сцены заменён, но сводка в память НЕ сделана (архивариус не ответил или ответил нераспознаваемо). Следующие сцены не будут знать её событий — стоит перезапустить сцену или дописать сводку руками во вкладке «Память».');
      }catch(e){
        console.warn('summarize failed (chat apply)', e);
        alert('Текст сцены заменён, но сводку в память сделать не удалось: '+e.message+'\nСледующие сцены не будут знать её событий.');
      }
    })();
  };
