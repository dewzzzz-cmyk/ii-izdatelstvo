  ...
  els.center.innerHTML = `
    <div class="hero">
      ...
      <div id="adv" style="${_conceptAdvOpen?'':'display:none'}">
        ...
        ${/* «примерно» — не смягчение формулировки, а факт: число глав уходит
             Архитектору пожеланием, а не ограничением. Живой прогон: просил 3
             главы, получил 4 — и это правильно, материал не обязан делиться
             ровно. Прежнее «Число глав» обещало точность, которой нет. */''}
        <div class="field"><label>Глав (примерно) <span class="hint">пожелание Архитектору, не жёсткое ограничение: он может дать на главу-две больше, если того требует арка. Пусто/0 = «авто» (предзаполнит поле на стадии Структура, там можно переопределить точечно)</span></label>
  ...
  els.center.innerHTML = `
    <div class="scene-bar">
      ...
      ${/* 0.6 → 0.7: ровно тот же порог, по которому САМ пайплайн считает сцену
            ...
            60-70% система внутри себя боролась за объём, а автор об этом
            не знал — бейдж молчал. */''}
      ${(()=>{
        // Порог бейджа обязан совпадать с порогом противовеса в pipeline.js
        // (tooShort = targetWords*0.9). Он отстал на 0.7 — и полоса 70-90%
        // оставалась немой: живой прогон закрыл сцену на 544 из 675 (81%),
        // и в интерфейсе это выглядело обычной готовой сценой. Хуже того,
        // именно в этой полосе противовес и работает, то есть система внутри
        // себя борется за объём, а автор об этом не знает.
        if(!(scene.status==='done' && scene.targetWords && scene.words)) return '';
        if(scene.words >= scene.targetWords*0.9) return '';
        const доля = Math.round(scene.words/scene.targetWords*100);
        return `<span class="stale-badge" title="Сцена короче цели на ${100-доля}%. Так бывает и намеренно (короткая сцена держит ритм), и из-за накопленных сокращений — правки Оценщика и Стражей режут текст, а вернуть объём некому. Проверьте на слух; если коротко не задумано — «Переписать» с указанием развернуть.">📏 ${scene.words} из ${scene.targetWords} слов · ${доля}%</span>`;
      })()}
      ${(()=>{
        // Балл Оценщика прямо на экране редактора, а не только в списке сцен слева.
        // Живой инцидент: сцена закрылась с 7.3 при пороге 7.5 (pass:false, три
        // итерации исчерпаны) и получила статус «готово» — а автор, читающий текст
        // в редакторе, об этом не узнавал никак: бейдж балла существовал только в
        // левой панели, которая на узком экране вообще скрыта (adaptive-макет).
        // Принять недотянувшую до собственного порога сцену, не заметив, было слишком легко.
        const v = scene.lastEval;
        if(!v || typeof v.weighted !== 'number') return '';
        const порог = s.global?.evaluatorThreshold ?? 7.5;
        const прошла = v.pass !== false && v.weighted >= порог;
        return `<span class="sr-score ${прошла?'score-pass':'score-fail'}" title="${прошла
          ? `Оценщик: ${v.weighted} — порог ${порог} пройден.`
          : `Оценщик: ${v.weighted} при пороге ${порог} — сцена НЕ дошла до порога, попытки доработки закончились. Стоит перезапустить агентов, поправить руками или снизить порог в настройках.`}">${прошла?'★':'⚠'} ${v.weighted}</span>`;
      })()}
...
async function doRun(els, s, scene, directive, runFlags={}){
  const g=s.global;
  if(_busy) return;
  // Раньше здесь проверялся голый s.global.apiKey — тот же класс бага, что уже
  // чинили в ondemand.js/world.js/illustrations.js/bookreview.js/architect-book.js
  // (аудит 1.70.0): Прозаик мог быть полностью настроен на свой провайдер через
  // llmFor() (без единого символа в глобальном ключе), а автора всё равно
  // останавливали с «Не задан API-ключ». Прозаик — единственный агент, без
  // которого сцену физически не с чем начать (остальные Стражи/Оценщик
  // деградируют по одному, не роняя прогон — см. pipeline.js), поэтому
  // достаточно проверить именно его резолвленный ключ, тем же приёмом, что
  // уже стоит на applyInlineEdit чуть ниже по файлу.
  if(!llmFor(s, ag(s,'prose')).apiKey){ alert('Задайте API-ключ в настройках (⚙) — либо глобальный, либо персональный для роли «Прозаик».'); return; }
