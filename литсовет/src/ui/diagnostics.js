...
function paramSpecs(a){
  ...
  if(a.role!=='bookArchitect'){
    // Хинт и def были написаны до +20%-прохода по лимитам этого окна и до
    // текущих значений auto-retry-на-обрыв в pipeline.js — теперь разные у
    // каждой роли (Прозаик 4320, ретрай до 19200; Оценщик 3840, ретрай до
    // 7200; Стражи 2520, ретрая на обрыв нет вовсе). Общий текст на все роли
    // был неточен и вводил в заблуждение — раскрываем нужное под роль.
    const retryHint = a.role==='prose'
      ? 'при обрыве Прозаик автоматически повторяет с удвоенным лимитом, максимум 19200'
      : a.role==='evaluator'
      ? 'при обрыве Оценщик автоматически повторяет с удвоенным лимитом (не ниже текущего), максимум 19200'
      : a.role==='lineedit'
      ? 'при обрыве Линейный редактор автоматически повторяет с лимитом по длине текста'
      : 'авто-повтора на обрыв у этой роли нет — при нехватке лимита ответ просто не распарсится';
    specs.push({ key:'maxTokens', label:'Макс. токенов', hint:`потолок длины ответа (${retryHint})`, min:200, max:8000, step:100, target:'agent', def:a.maxTokens??2520, fmt:v=>Math.round(v) });
  }
  if(a.role==='evaluator'){
  ...
  if(a.role==='bookArchitect'){
    ...
    // перегенерации хвоста) показал, что автору всё равно иногда нужна ручка
    // «дать ещё больше места», не дожидаясь очередного ручного бампа
    // константы в коде. Множитель применяется ПОВЕРХ авто-формулы во всех
    // трёх местах (генерация скелета, точечная правка, каскадная
    // перегенерация хвоста) — не отменяет масштабирование по объёму книги,
    // просто даёт больше запаса сверху.
    specs.push({ key:'architectTokenMultiplier', label:'Запас токенов', hint:'множитель поверх авто-рассчитанного потолка (по объёму книги) — поднимите, если скелет/каскадная правка обрываются на «не удалось распарсить JSON»', min:1, max:3, step:0.25, target:'global', def:1, fmt:v=>'×'+v.toFixed(2).replace(/\.?0+$/,'') });
  }
