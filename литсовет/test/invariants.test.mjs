...
// Живой скелет «Грез»: 18 глав по завязке в 45 слов. Три главы назывались
// «Возвращение в Пустошь», последние две — «Кульминация» и «Развязка», то есть
// модель назвала главы именами их же слотов. Оценщик структуры поставил 6.2 и
// выдал десять замечаний — но уже ПОСЛЕ того, как автор заплатил за генерацию.
test('одинаково названные главы отбраковывают скелет', async () => {
  const { validateSkeleton, findDuplicateChapterTitles } = await import('../src/architect-book.js');
  const сц = ()=>[{ title:'с', brief:'б', targetWords:900 }];
  const raw = JSON.stringify({ chapters:[
    { title:'Глава 1: Падение в новый мир',   arc:'завязка',  scenes:сц() },
    { title:'Глава 4: Возвращение в Пустошь', arc:'развитие', scenes:сц() },
    { title:'Глава 6: Возвращение в Пустошь', arc:'развитие', scenes:сц() },
    { title:'Глава 9: Тишина',                arc:'развязка', scenes:сц() },
  ]});
  const res = validateSkeleton(raw);
  assert.equal(res.ok, false, 'скелет с тремя одинаковыми главами не должен приниматься');
  assert.match(res.error, /названы одинаково/);
  assert.match(res.error, /Возвращение в Пустошь/, 'автору нужно название, а не только номера');

  // Префикс «Глава N:» модель ставит сама — сравнивать заголовки как есть бесполезно.
  const группы = findDuplicateChapterTitles([
    { title:'Глава 4: Возвращение в Пустошь' },
    { title:'Глава 6: возвращение в пустошь!' },
  ]);
  assert.equal(группы.length, 1, 'префикс и регистр не должны прятать дубль');
  assert.deepEqual(группы[0].numbers, [1,2]);

  // Нумерация та же, что видит автор: пролог не сдвигает номера глав.
  const сПрологом = findDuplicateChapterTitles([
    { title:'Пролог', arc:'пролог' },
    { title:'Глава 1: Падение', arc:'завязка' },
    { title:'Глава 2: Возвращение', arc:'развитие' },
    { title:'Глава 3: Возвращение', arc:'развитие' },
  ]);
  assert.deepEqual(сПрологом[0].numbers, [2,3], 'пролог в счёт глав не идёт');
});

test('глава, названная именем своей дуги, отбраковывает скелет', async () => {
  const { validateSkeleton, titleIsArcLabel } = await import('../src/architect-book.js');
  const сц = ()=>[{ title:'с', brief:'б', targetWords:900 }];
  const res = validateSkeleton(JSON.stringify({ chapters:[
    { title:'Ящик',       arc:'завязка',     scenes:сц() },
    { title:'Кульминация',arc:'кульминация', scenes:сц() },
    { title:'Развязка',   arc:'развязка',    scenes:сц() },
  ]}));
  assert.equal(res.ok, false);
  assert.match(res.error, /имя слота структуры/);

  // Пролог и эпилог обязаны называться именно так — FRAME_CHAPTERS_NOTE этого требует.
  assert.equal(titleIsArcLabel('Пролог', 'пролог'), false, 'иначе правило само себе противоречит');
  assert.equal(titleIsArcLabel('Эпилог', 'эпилог'), false);
  assert.equal(titleIsArcLabel('Кульминация', 'кульминация'), true);
  assert.equal(titleIsArcLabel('Гонка со временем', 'кульминация'), false, 'нормальное название не трогаем');
});

// Замер на тех же 69 брифах: выбор сформулирован в 48, а конкретная цена —
// только в 16. Остальное сползает в пересказ, и Прозаик пишет сводку событий.
test('брифы-пересказы находятся, а брифы с ценой — нет', async () => {
  const { findWeakBriefs } = await import('../src/architect-book.js');
  const слабые = findWeakBriefs([
    { id:'a', title:'Новая жизнь',  brief:'Даня узнаёт, что в городе люди теряют воспоминания. Он решает расследовать это и побеждает вора.' },
    { id:'b', title:'Отец',         brief:'Отец объясняет Дане, что тот может использовать силу, чтобы помогать людям. Даня соглашается.' },
    { id:'c', title:'Шахта',        brief:'Даня узнаёт от шахтёров, что за добычу Морок берёт половину. Он лезет в шахту сам, рискуя тем, что кристалл заберёт его воспоминания о матери — единственное, что у него осталось.' },
    { id:'d', title:'Погоня',       brief:'Даня бежит через Пустошь, за ним идёт тень. Он бросает очки в трещину, чтобы отвлечь её.' },
  ]);
  const ids = слабые.map(w=>w.id);
  assert.deepEqual(ids, ['a','b'], 'пересказ ловим, а сцену с ценой и сцену действия — нет');
  assert.equal(слабые[0].title, 'Новая жизнь', 'автору нужно название сцены, чтобы её найти');
});

// Ретрай на содержательную ошибку не должен просить «верни строго JSON»:
// формат-то был в порядке, и такая подсказка уводит модель не туда.
test('содержательная ошибка скелета помечена отдельно от формата', async () => {
  const { validateSkeleton } = await import('../src/architect-book.js');
  const сц = ()=>[{ title:'с', brief:'б', targetWords:900 }];
  const дубль = validateSkeleton(JSON.stringify({ chapters:[
    { title:'Возвращение', arc:'завязка',  scenes:сц() },
    { title:'Возвращение', arc:'развязка', scenes:сц() },
  ]}));
...
// Правило дуг жило только в промпте — в коде его не проверял никто. Живой
// прогон детектива (16 глав) вскрыл провал, который стал достижим именно
// после добавления рамочных глав: пролог → завязка → развитие ×13 →
// КУЛЬМИНАЦИЯ → ЭПИЛОГ. Развязки нет вообще, книга обрывается на пике.
// Модель послушалась инструкции «кульминация предпоследняя» — но предпоследней
// среди ВСЕХ глав, считая эпилог, и развязку вытеснило.
test('скелет без развязки отбраковывается', async () => {
  const { validateSkeleton, validateOutline } = await import('../src/architect-book.js');
  const гл = (title, arc) => ({ title, arc, turn:'нечто меняется навсегда', scenes:[{ title:'с', brief:'б', targetWords:900 }] });

  const без = { chapters:[
    гл('Пролог','пролог'), гл('Сердце не болит','завязка'), гл('Все свои','развитие'),
    гл('Первый отдел','развитие'), гл('Десять лет тишины','кульминация'), гл('Эпилог','эпилог'),
  ]};
  const v = validateSkeleton(JSON.stringify(без));
  assert.equal(v.ok, false, 'книга без развязки не должна приниматься');
  assert.equal(v.kind, 'content');
  assert.match(v.error, /нет главы с дугой «развязка»/);
  assert.match(v.error, /Эпилог развязку не заменяет/, 'модели надо объяснить, почему эпилог не считается');

  // Та же проверка на ПЕРВОМ проходе — там отказ стоит центы, а не всю книгу.
  const o = validateOutline(JSON.stringify({ chapters: без.chapters.map(({title,arc,turn})=>({title,arc,turn})) }));
  assert.equal(o.ok, false);
  assert.match(o.error, /развязка/);

  // Полная арка с рамкой принимается.
  const целая = { chapters:[
    гл('Пролог','пролог'), гл('Сердце не болит','завязка'), гл('Все свои','развитие'),
    гл('Десять лет тишины','кульминация'), гл('Справка о смерти','развязка'), гл('Эпилог','эпилог'),
  ]};
  assert.equal(validateSkeleton(JSON.stringify(целая)).ok, true);

  // Две кульминации — тоже ошибка; короткая книга из 2 глав не проверяется.
  const две = { chapters:[гл('А','завязка'), гл('Б','кульминация'), гл('В','кульминация'), гл('Г','развязка')] };
  // При ровно трёх главах кульминация совпадает с развязкой (так сказано в
  // ARC_DISTRIBUTION_NOTE) — отдельного пика требовать нельзя.
  assert.equal(validateSkeleton(JSON.stringify({ chapters:[гл('А','завязка'), гл('Б','развитие'), гл('В','развязка')] })).ok, true,
    'книга из трёх глав не обязана иметь отдельную кульминацию');
  assert.match(validateSkeleton(JSON.stringify(две)).error, /кульминация.*ровно одна/);
