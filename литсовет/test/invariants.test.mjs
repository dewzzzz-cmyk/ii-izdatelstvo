...
// Метроном в скелете создавала не модель, а наша же строка «~N сцен на главу»:
// в двух живых прогонах «Грез» ВСЕ главы получили ровно по 4 сцены.
// И не было ни одного правила, запрещающего главе повторять уже пройденное.
test('Архитектор не задаёт равное число сцен и запрещает витки', async () => {
  const { bookArchitectMessages } = await import('../src/architect-book.js');
  const state = { project:{ title:'Т', genre:'фэнтези', synopsis:'С', targetWords:80000 }, agents:{}, structure:[], bible:[] };
  const весь = bookArchitectMessages(state).map(m=>m.content).join('\n');
  assert.ok(!/~\d+ сцен на главу/.test(весь),
    'фиксированное число сцен на главу — это и есть источник равномерного ритма');
  assert.match(весь, /НЕРАВНОМЕРНО/, 'разброс должен требоваться явно');
  assert.match(весь, /НЕОБРАТИМОСТЬ/, 'без правила о необратимости глава может вернуть книгу назад');
  assert.match(весь, /Переименование не считается решением/,
    'буквальные дубли ловит код, переименованные — только это правило');
  assert.match(весь, /имеешь право вернуть меньше глав/,
    'иначе модель разбавляет сюжет витками, лишь бы добрать запрошенное число');
});
